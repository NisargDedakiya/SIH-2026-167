...
const DEMO_SCENARIOS: DemoScenario[] = [
  {
    ...
    description: "Evaluates multi-spectral satellite imagery to identify dominant land cover categories, density, and natural terrain features.",
    icon: Layers,
    defaultQuery: "What type of land cover dominates this satellite image?",
    alternateQueries: [
  ...
  },
  {
    ...
    description: "Localizes specific physical entities using natural language directives, mapping pixel bounding boxes directly to native UTM geospatial coordinates.",
    icon: Crosshair,
  ...
  },
  {
    ...
    description: "Aligns pre- and post-event satellite acquisitions (T1 & T2), performs spatial resampling, and generates quantitative change difference maps.",
    icon: GitCompare,
  ...
  },
  {
    ...
    description: "Fuses cloud-penetrating Synthetic Aperture Radar (SAR) backscatter with multi-spectral optical data to detect all-weather structures and water boundaries.",
    icon: Radio,
...
export default function DemoHubPage() {
  ...
  const executeDemo = async () => {
    try {
      ...
      // Fetch latest images from registry to bind real imagery
      const imagesList = await listImages(10);
      const availableIds = imagesList.map((img) => img.id);

      // Execute via agent router
      const neededCount = selectedDemo.id === "demo-3" || selectedDemo.id === "demo-4" ? 2 : 1;
      const res = await analyzeAgentMulti(availableIds.slice(0, neededCount), activeQuery);

      setAnalysisResult(res);
    } catch (err: any) {
      setError(err.message || "Failed to execute demonstration analysis.");
    } finally {
      setIsRunning(false);
    }
  };
    ...
    <AppShell>
      <div className="space-y-8">
        ...
        <div className="rounded-xl border border-slate-800 bg-surface/60 p-6 space-y-6">
          ...
          {/* Error Message */}
          {error && (
            <div className="flex items-center space-x-3 p-4 rounded-lg bg-rose-950/40 border border-rose-800/50 text-rose-300 text-sm">
