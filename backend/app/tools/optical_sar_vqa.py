"""
Optical-SAR Cross-Modal Visual Question Answering (VQA) Tool.
Answers natural language analytical inquiries targeting modality complementarity,
distinct physical signatures, and feature verification.
"""
...
class OpticalSARVQATool(AnalysisTool):
    """
    Standardized AnalysisTool wrapper for Optical-SAR Cross-Modal VQA.
    """

    def __init__(self):
        self.name = "optical_sar_vqa"
        self.version = "1.0.0"
        self.task = "CROSS_MODAL_ANALYSIS"
        self.description = (
            "Answers natural language questions about complementary phenomena visible "
            "across co-registered Optical and SAR imagery, explaining physical scattering versus spectral signatures."
        )
        ...
        self.optional_parameters = {
            "query": "What does SAR reveal that is difficult to see in the optical image?",
        }
    ...
    def validate_inputs(
        ...
        if not has_pair and len(image_ids) < 2:
            raise ValueError(
                f"Tool '{self.name}' requires either a 'pair_id' or 2 satellite image IDs (Optical and SAR)."
            )
    ...
    async def execute(
        ...
        query = parameters.get("query") or input_context.get("query") or "What does SAR show that is difficult to see in the optical image?"
